    ...
    def findFaceMesh(self, img, draw=True):
        ...
        if results.multi_face_landmarks:
            ...
            for idx, faceLms in enumerate(results.multi_face_landmarks):
                ...
                face_normalized = []
                for id, (x, y) in zip(self.ALL_LANDMARKS, zip(x_list, y_list)):
                    x_norm = (x - x_min) * scale
                    y_norm = (y - y_min) * scale
                    face_normalized.append((id, int(x_norm), int(y_norm)))

                normalized_img = np.zeros((48, 48, 3), dtype=np.uint8)
                for id, x, y in face_normalized:
                    cv2.circle(normalized_img, (x, y), 1, (255, 255, 255), -1)
